...
1. **Pick the model.** Match the user's intent to a model from the live roster. Discover it live, never from memory: MCP `list_models` (filter by `contentType`) or CLI `contenthero model list --type <image|video|audio>`; the roster is also the `modelId` enum on the `generate_*` tools, and an out-of-roster id is rejected. Selection guidance and the current snapshot are in `references/model-catalog.md`.
2. **Shape the request.** Fetch the model's real request shape with MCP `get_model <modelId>` / CLI `contenthero model get <id>`, then set only the parameters it supports (aspect, resolution, duration, references, model-specific mode/shots). When the model takes references, read its `promptReferences`: it tells you whether and how to address each reference in the prompt for that specific model (numbered `@Image1` tags, named `@element` tags, plain "image 1" prose, or describe-by-role). Use the scheme the model declares; using one it does not just drops stray text into the prompt. Reference inputs and tagging, field by field, are in `references/media-inputs.md`. **For prompt quality, read `references/prompt-craft.md`** once you know the model. A well-crafted prompt is the difference between a usable result and a wasted spend.
