...
const derived = {
  ...
  // Cursor's "From GitHub Repository" import refuses a repo without this file, even though Cursor
  // loads the plugin itself from the root Agent Plugins manifest (cursor.com/docs/skills, 2026-09-24).
  '.cursor-plugin/marketplace.json': pretty({
    name: plugin.name,
    owner: { name: plugin.author?.name },
    metadata: { description: plugin.description },
    plugins: [{ name: plugin.name, source: './', description: plugin.description, version: plugin.version, logo: `./${SKILL}/assets/icon.svg` }],
  }),
...
if (mode === '--zip') {
  ...
  zip('contenthero-plugin.zip', root, ['plugin.json', 'mcp.json', '.mcp.json', '.claude-plugin', '.cursor-plugin', 'skills', 'README.md', 'LICENSE'])
